...
> **CRITICAL:** This command only edits `.claude/CLAUDE.md` (the text file).
> It **NEVER** deletes, modifies, or moves any file inside `.claude/agents/`, `.claude/skills/`,
> `.claude/commands/`, or `.claude/kb/`. Those directories are always left untouched.
...
**Replace**: Fully regenerate from source (text in CLAUDE.md only — never file deletion)
**Merge**: Add new, preserve custom
**Preserve**: Never auto-modify

> All "Replace" and "Merge" operations apply **only to text sections inside CLAUDE.md**.
> No files inside `.claude/agents/`, `.claude/skills/`, `.claude/commands/`, or `.claude/kb/` are ever created, edited, or deleted by this command.
