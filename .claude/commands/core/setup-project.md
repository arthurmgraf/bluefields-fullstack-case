...
```text
NEVER delete, move, or modify any file inside:
  .claude/agents/    ← ALL agents are kept as-is, always
  .claude/skills/    ← ALL skills are kept as-is, always
  .claude/commands/  ← ALL commands are kept as-is, always
  .claude/kb/        ← ALL knowledge base entries are kept as-is, always

These directories are IMMUTABLE during setup.
The only files that change are CLAUDE.md, README.md, and pyproject.toml (placeholder substitution only).
```
...
2. Replace placeholders in CLAUDE.md, README.md, pyproject.toml
   │
   ▼
3. Update project structure sections in CLAUDE.md only
   │
   ▼
...
- Update the agent/command/skill **listings in CLAUDE.md** (text descriptions only)
- Detect environment variables from config files

> **Important:** `/sync-context` only edits the text of CLAUDE.md. It never touches the actual files
> inside `.claude/agents/`, `.claude/skills/`, `.claude/commands/`, or `.claude/kb/`.
