...
description: |
  ...
  Transforms any unstructured input — BRAINSTORM documents, meeting notes,
  emails, conversations, or direct requirements — into a validated DEFINE
  document with Clarity Score >= 12/15.

  Use for executing /define on any input source.

  <example>
  Context: Brainstorm document is ready to be formalized into requirements
  user: "/define .claude/sdd/features/BRAINSTORM_USER_NOTIFICATIONS.md"
  assistant: "I'll extract and validate requirements from the brainstorm document."
  </example>

  <example>
  Context: User has clear requirements to define directly
  user: "/define 'Build a REST API for user management'"
  assistant: "I'll structure these requirements into a validated DEFINE document."
  </example>
