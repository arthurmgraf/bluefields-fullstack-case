...
Images are embedded as base64 data URLs in the `files` object and referenced by a **hash-based `fileId`** in image elements.

**CRITICAL:** The `fileId` MUST be a hash string (not a data URL). Excalidraw looks up `fileId` in the `files` object to find the image data. Using a data URL directly as `fileId` will NOT render.
...
```json
{
  ...
  "link": null,
  "locked": false,
  "fileId": "a45f3866ceb2b5767e2a228bafd284b6297f05a0",
  "status": "saved",
  "scale": [1, 1]
}
```
...
| Property | Type | Description |
|----------|------|-------------|
| `fileId` | string | SHA-256 hash (40 chars) referencing a key in the `files` object. **Never** a data URL. |
| `status` | string | Always `"saved"` |
| `scale` | array | [scaleX, scaleY], typically [1, 1] |

### Files Object Structure

The root-level `files` object stores embedded image data, keyed by the same hash used in `fileId`:

```json
{
  "files": {
    "a45f3866ceb2b5767e2a228bafd284b6297f05a0": {
      "mimeType": "image/svg+xml",
      "id": "a45f3866ceb2b5767e2a228bafd284b6297f05a0",
...
Generate the fileId hash from the data URL:
...
This ensures:
- Deterministic: same logo always gets the same hash
- Unique: different logos get different hashes
- Deduplication: same logo used multiple times only stored once in `files`
...
| `id` | string | Same hash as the key in files object and the element's fileId |
| `dataURL` | string | Complete base64 data URI (e.g., `data:image/svg+xml;base64,...`) |
| `created` | number | Unix timestamp in milliseconds |
| `lastRetrieved` | number | Unix timestamp in milliseconds |
...
See `diagrams/generate_diagrams.py` — functions `load_logos()`, `make_image()`, and `save_excalidraw()` implement this pattern end-to-end.
...
```json
{
  ...
  "files": {
    "a45f3866ceb2b5767e2a228bafd284b6297f05a0": {
      "mimeType": "image/svg+xml",
      "id": "a45f3866ceb2b5767e2a228bafd284b6297f05a0",
      "dataURL": "data:image/svg+xml;base64,PHN2Zy...(base64 encoded SVG)...",
      "created": 1740000000000,
      "lastRetrieved": 1740000000000
    }
  }
...
> **Note:** If the diagram has no images, `"files": {}` is acceptable.
> If the diagram HAS images, every image element's `fileId` MUST have a
> matching entry in `files`, otherwise the image will not render.

---

**Last Updated:** 2026-02-24
**Format Version:** Excalidraw v2 — Hash-based fileId for images (mandatory)
